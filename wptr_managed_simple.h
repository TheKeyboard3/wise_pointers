#ifndef WPTR_MANAGED_SIMPLE_H
#define WPTR_MANAGED_SIMPLE_H
#include "wptr_managed.h"
/**
 * A template class that contains an object managed by wise_pointers
 */
template<typename T>
class wptr_managed_simple : public wptr_managed<T>
{
protected:
    T managed_obj;
...
public:
     ...
     */
    wptr_managed_simple(const T& obj){
        managed_obj = T(obj);
    }
     ...
     */
    wptr_managed_simple(const wptr_managed_simple& other){
     ...
     * Destructor
     */
    virtual ~wptr_managed_simple() override{
